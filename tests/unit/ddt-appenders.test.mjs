...
// appender 会自己 mkdir docs/ssot，测试无需预建。
function newRepo() {
  return mkdtempSync(path.join(os.tmpdir(), 'ddt-ap-'));
}

test('decisions-append：单条 JSON 写入 docs/ssot/decisions.jsonl 并自动加 ts', () => {
  const dir = newRepo();
  ...
  const lines = readFileSync(path.join(dir, 'docs/ssot/decisions.jsonl'), 'utf8').trim().split('\n');
...
test('changelog-append：写 docs/ssot/changelog.jsonl 自动加 ts', () => {
  const dir = newRepo();
  const r = spawnSync('node', [clAppender], {
    input: JSON.stringify({ kind: 'amend', intent: 'add field', paths: ['docs/ssot/prd.md'] }),
    cwd: dir, encoding: 'utf8'
  });
  assert.equal(r.status, 0);
  const row = JSON.parse(readFileSync(path.join(dir, 'docs/ssot/changelog.jsonl'), 'utf8').trim().split('\n')[0]);
