...
3. **Ingest**：产物（代码/URL/figma）落 `docs/design/frontend/`（bundle 的家），在 `.ddt/changelog.jsonl` 记一条来源（工具/来源/人/时间），再走 `ddt-brainstorming` 把它落成前端 design spec。
...
- **消费端**：各前端切片**固定来 `docs/design/frontend/` 读**，不猜位置。
- **opt-out**：无前端 / 前端极简到不值得外部设计 / 没有外部设计工具 → 记一条 decision「本期前端不外部设计，理由…」。判定：**目录空 + 无 opt-out = 该触发本回路；非空 = 消费；有 opt-out = 用设计系统直接实现**。别假装走过。

## 关系

上游 `大需求变小`（页面清单）→ 本回路出 bundle 落 `docs/design/frontend/` → `ddt-brainstorming` 落前端 spec → 各前端切片 `ddt-writing-plans` / `ddt-subagent-driven` **消费 `docs/design/frontend/`** 实现 → `ddt-requesting-review`。
