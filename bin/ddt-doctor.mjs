...
console.log(`  ${existsSync(path.join(cwd, '.ddt')) ? '✓' : '✗'} .ddt/                    （DDT 工作态目录）`);
console.log(`  ${existsSync(path.join(cwd, '.ddt/decisions.jsonl')) ? '✓' : '✗'} .ddt/decisions.jsonl     （SSoT 决策账本）`);
console.log(`  ${existsSync(path.join(cwd, '.ddt/changelog.jsonl')) ? '✓' : '✗'} .ddt/changelog.jsonl     （SSoT 变更账本）`);
console.log(`  ${existsSync(path.join(cwd, '.ddt/state/current.json')) ? '✓' : '✗'} .ddt/state/current.json  （command→hook 字段桥）`);
console.log(`  ${existsSync(path.join(cwd, 'docs/specs')) ? '✓' : '✗'} docs/specs/              （切片 spec 目录）`);
